...
#define DEFAULT_DEGREES 90.0f

#define AILERON_MIN  -50.0f //todo figure out what these actual values are
#define AILERON_MAX  50.0f
#define ELEVATOR_MIN -50.0f
#define ELEVATOR_MAX 50.0f
#define RUDDER_MIN   -50.0f
#define RUDDER_MAX   50.0f
...
int lerShifts[3] = {1,2,4};
volatile uint32_t* matchRegisters[3] = {&(LPC_PWM1->MR1),&(LPC_PWM1->MR2),&(LPC_PWM1->MR4)};
...
int servos_SetAngle(servo_channel_t channel, float degrees)
{
	...
	if((degrees>=degreesMin[(int)channel])&&(degrees<=degreesMax[(int)channel]))
	{
		...
		*(matchRegisters[(int)channel]) = (int)(((degrees+180.0f)*500.0f)/3.0f);
		LPC_PWM1->LER |= (1<<lerShifts[channel]);
		return 0;
	} else {
...
int servos_SetThrust(float thrust_pct)
{
	...
	{
		LPC_PWM1->MR3 = (int)((thrust_pct+100.0f)*300.0f);
		LPC_PWM1->LER |= (1<<3);
		return 0;
	}else{
